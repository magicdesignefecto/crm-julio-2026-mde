...
export const LeadsModule = {
                    ...
                    <div style="display:flex; flex-direction:column; gap:10px;">
                        <div>
                            ...
                            <select id="billingService" class="form-select" style="width:100%; margin-top:4px;">
                                ...
                                ${availableServices.map(svc => `<option value="${svc.name}" data-price="${svc.price || 0}">${svc.name} (Bs. ${Number(svc.price || 0).toLocaleString('es-ES')})</option>`).join('')}
                            </select>
                        </div>
    ...
    init: async () => {
        ...
        // Auto-rellenar monto de billing al seleccionar servicio
        const billingServiceEl = document.getElementById('billingService');
        const billingAmountEl = document.getElementById('billingAmount');
        if (billingServiceEl && billingAmountEl) {
            billingServiceEl.addEventListener('change', () => {
                const selected = billingServiceEl.options[billingServiceEl.selectedIndex];
                if (selected && selected.dataset.price) {
                    const price = Number(selected.dataset.price);
                    if (price > 0) billingAmountEl.value = price;
                }
            });
        }
        ...
        try {
            ...
            container.querySelectorAll('.btn-edit-lead').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    document.getElementById('displayTotal').innerText = `${lead.currency} ${(lead.total || 0).toLocaleString('es-BO', { minimumFractionDigits: 2 })}`;
    ...
    viewLead: async (id) => {
        ...
        container.innerHTML = `
            <div style="display:flex; flex-direction:column; gap:16px;">
                <!-- HEADER -->
                <div style="display:flex; gap:12px; align-items:center; border-bottom:1px solid var(--border-color, #F3F4F6); padding-bottom:15px; flex-wrap:nowrap;">
                    <div style="width:46px; height:46px; min-width:46px; background:#E0E7FF; color:#4338CA; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.1rem; font-weight:700; flex-shrink:0;">${lead.name.charAt(0)}</div>
                    <div style="flex:1; min-width:0; overflow:hidden;">
                        <h3 style="margin:0; color:var(--text-main, #1F2937); font-size:1.05rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${lead.name}</h3>
                        <p style="margin:2px 0 0 0; color:var(--text-muted, #6B7280); font-size:0.85rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${lead.company || 'Particular'}</p>
                    </div>
                    <span style="background:${stColor}20; color:${stColor}; padding:5px 12px; border-radius:20px; font-size:0.75rem; font-weight:700; border:1px solid ${stColor}40; white-space:nowrap; flex-shrink:0;">${lead.status}</span>
                </div>
