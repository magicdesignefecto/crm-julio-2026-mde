...
import { collection, getCountFromServer, getDocs, query, where, doc, updateDoc, runTransaction } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
    ...
    getData: async (forceRefresh = false) => {
        try {
            ...
            const revenue = allLeads
                ...
                .reduce((acc, lead) => acc + (Number(lead.total) || 0), 0);
            ...
            allLeads.forEach(lead => {
                ...
                // *** INICIO: calcular desde la fecha de registro y avanzar por frecuencia ***
                // Empezar desde el mes de registro del lead
                let current = new Date(leadDate.getFullYear(), leadDate.getMonth(), 1);

                // Si el día de cobro cae antes de la fecha de registro en el mes de registro,
                // el primer cobro es el siguiente ciclo
                ...
                // Avanzar hasta llegar al período relevante (no generar instancias muy antiguas)
                // Mostramos: 1 mes atrás (para overdue reciente) hasta futureLimit
                const displayStart = new Date(today.getFullYear(), today.getMonth() - 1, 1);
                while (current <= futureLimit) {
                    const lastDay = new Date(current.getFullYear(), current.getMonth() + 1, 0).getDate();
                    const day = Math.min(dayOfMonth, lastDay);
                    const checkDate = new Date(current.getFullYear(), current.getMonth(), day);
                    if (checkDate >= displayStart) break; // Esta instancia ya es relevante
                    current.setMonth(current.getMonth() + monthStep);
                }
                ...
                while (current <= futureLimit) {
                    ...
                    // Gracia: si el cobro cae en el mismo mes de registro del lead,
                    // NO marcar como vencido (el adelanto cubre ese primer mes)
                    const isFirstMonth = (instanceDate.getFullYear() === leadDate.getFullYear() && instanceDate.getMonth() === leadDate.getMonth());

                    let status = 'upcoming';
                    if (diffDays < 0) {
                        // Solo aplicar gracia si el lead tiene adelanto pagado
                        status = (isFirstMonth && b.advance > 0) ? 'soon' : 'overdue';
                    } else if (diffDays <= 3) {
                        status = 'soon';
                    }
