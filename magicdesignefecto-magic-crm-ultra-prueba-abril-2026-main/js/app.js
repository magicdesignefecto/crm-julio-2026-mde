...
const lazyModule = (importFn) => {
    ...
    return async () => {
        if (!cached) {
            ...
            // Buscar el export que tenga render() e init() (es el Module)
            cached = Object.values(mod).find(exp => exp && typeof exp.render === 'function' && typeof exp.init === 'function')
                || Object.values(mod)[0];
        }
        return cached;
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const _dashboardPreload = import('./modules/dashboard.js').catch(() => { /* preload silencioso */ });
